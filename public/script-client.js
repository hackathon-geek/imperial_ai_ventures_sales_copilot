...
//                     aiRecommendationsElement.setHTMLUnsafe(`<li>${aiResponse}</li>`);
//                     break;
//                 }
//                 case "response.done":
...
const videoGridElement = document.getElementById('client-video');
...
const addOtherVideoStream =  (videoElement, stream) => {
    videoElement.srcObject = stream;
    videoElement.addEventListener('loadedmetadata', () => {
        videoElement.play();
    })
...
const connectToNewUser = (userId, stream) => {
    ...
    callObj.on('stream', userVideoStream => {
        addOtherVideoStream(videoElement, userVideoStream)
    })
